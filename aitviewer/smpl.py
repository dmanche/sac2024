    ...
    def from_amass(
        ...
        **kwargs,
        ...
        if log:
            ...
            print("FPS {}".format(body_data["mocap_framerate"])) # TODO: Es posible que en algunos casos, esta linea de error debido a que el diccionario incluya otro nombre para la clave
            # relacionada con el frame rate. En mi caso, tuve que cambiar mocap_framerate a mocap_frame_rate en algunas ocasiones.
        ...
        if fps_out is not None:
            fps_in = body_data["mocap_framerate"].tolist() #TODO: En esta linea, el problema es el mismo que el mencionado justo arriba.
            if fps_in != fps_out:
        ...
        #TODO: Se modificaron las dos lineas siguientes para que no dieran error en caso de que no existieran los joints de las manos, que puede ocurrir si usamos el modelo SMPL, en lugar del SMPL-H.
        i_left_hand_end = i_body_end + (smpl_layer.bm.NUM_HAND_JOINTS * 3 if hasattr(smpl_layer.bm, 'NUM_HAND_JOINTS') else 3)
        i_right_hand_end = i_left_hand_end + (smpl_layer.bm.NUM_HAND_JOINTS * 3 if hasattr(smpl_layer.bm, 'NUM_HAND_JOINTS') else 3)
